...
void	Form::checkExec( const Bureaucrat& executor ) const
{
	if (!_signed)
		throw NotSigned();
	if (executor.getGrade() > _lowestGrade_toExec)
...
{
	out << "\033[1;33m" << value.getName() << "\033[0m" << " -> Grade requierd to sign : "
		<< "\033[1;31m" << value.getLowestGrade_toSign() << "\033[0m" 
		<< "\n			Grade requierd to execute "
		<< "\033[1;32m" << value.getlowestGrade_toExec() << "\033[0m" << std::endl;
	return ( out );
}